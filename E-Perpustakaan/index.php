...
<section id="statistik" class="section-pad stats-section">
  <div class="mx-auto max-w-7xl px-5 lg:px-8">
      ...
      <div class="mx-auto max-w-7xl px-5 lg:px-8">
        ...
        <div class="mb-10">
            <form id="searchForm" class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <input type="text" id="searchInput" placeholder="Cari judul atau pengarang..." 
                          class="w-full px-5 py-3 border rounded-xl focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20">
                </div>
                <div class="w-full md:w-48 relative">
                    <select id="kategoriFilter" class="w-full px-5 py-3 pr-10 border rounded-xl appearance-none cursor-pointer bg-white dark:bg-gray-800">
                ...
                <div class="w-full md:w-40 relative">
                    <select id="sortFilter" class="w-full px-5 py-3 pr-10 border rounded-xl appearance-none cursor-pointer bg-white dark:bg-gray-800">
                ...
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-[#f4d991] to-[#d6a84f] rounded-xl font-bold hover:scale-105 transition shadow-md hover:shadow-lg">
    ...
    <section id="pinjam" class="section-pad borrow-section">
      <!-- Di dalam section id="pinjam", tambahkan script ini setelah form -->
      <script>
      // Cek apakah ada parameter pinjam di URL
      const urlParams = new URLSearchParams(window.location.search);
      const pinjamBukuId = urlParams.get('pinjam');

      if (pinjamBukuId) {
          // Scroll ke form peminjaman
          document.getElementById('pinjam').scrollIntoView({ behavior: 'smooth' });
          
          // Tunggu select box terisi, lalu pilih buku yang sesuai
          setTimeout(() => {
              const bookSelect = document.getElementById('bookSelect');
              if (bookSelect) {
                  // Loop option dan pilih yang sesuai
                  for (let i = 0; i < bookSelect.options.length; i++) {
                      if (bookSelect.options[i].value == pinjamBukuId) {
                          bookSelect.value = pinjamBukuId;
                          bookSelect.dispatchEvent(new Event('change'));
                          
                          // Tambahkan highlight efek
      ...
      <div class="mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
        ...
        <form id="borrowForm" class="borrow-form glass-panel reveal" novalidate>
          <div class="form-grid">
            ...
            <label>
              <span>Email</span>
              <input type="email" name="email" placeholder="Email aktif untuk notifikasi pembayaran" maxlength="150" autocomplete="email" required>
            </label>
          </div>
          ...
          <section id="paymentPanel" class="payment-panel" hidden aria-live="polite">
            <div class="payment-panel-head">
              <div>
                <span>Pembayaran & Verifikasi</span>
                <strong id="paymentStatus">Menunggu pembayaran</strong>
              </div>
            ...
            <div class="payment-grid">
              <div class="payment-info">
                ...
                <button id="paymentReference" type="button" class="payment-reference" title="Klik untuk menyalin reference">-</button>
              </div>
              <div class="payment-info">
                <small>Nominal</small>
            ...
            <div id="paymentQrWrap" class="payment-qr-wrap" hidden>
              <img id="paymentQrImage" alt="QR pembayaran peminjaman" hidden>
              <pre id="paymentQrString" hidden></pre>
            </div>
            <div class="payment-actions">
              <a id="paymentOpenQr" class="secondary-button" href="#" target="_blank" rel="noopener" hidden>Buka QR <i class="fa-solid fa-up-right-from-square"></i></a>
              <button id="paymentCopyReference" type="button" class="secondary-button">Salin Reference <i class="fa-solid fa-copy"></i></button>
              <button id="paymentCheckStatus" type="button" class="secondary-button">Cek Status <i class="fa-solid fa-rotate"></i></button>
            </div>
          </section>
...
(function() {
    ...
    console.log('Pinjam Buku ID:', pinjamBukuId); // Cek di console browser
    
    if (pinjamBukuId && pinjamBukuId !== 'null') {
        ...
        function selectBook() {
            const bookSelect = document.getElementById('bookSelect');
            console.log('BookSelect ditemukan:', !!bookSelect);
            console.log('Jumlah option:', bookSelect?.options.length);
            
            if (bookSelect && bookSelect.options.length > 1) {
                let found = false;
                for (let i = 0; i < bookSelect.options.length; i++) {
                    const optionValue = bookSelect.options[i].value;
                    console.log('Option value:', optionValue, 'vs', pinjamBukuId);
                    if (optionValue == pinjamBukuId) {
                        ...
                        console.log('Buku terpilih:', bookSelect.options[i].text);
                        break;
                    }
                }
                if (!found) {
                    console.log('Buku dengan ID', pinjamBukuId, 'tidak ditemukan di select');
                }
            } else {
                // Jika select belum terisi, coba lagi
                setTimeout(selectBook, 500);
            }
        }
        
        // Pilih buku (dengan retry)
        setTimeout(selectBook, 500);
    }
})();
</script>
...
<section id="riwayat" class="section-pad history-section">
    ...
    <section id="riwayat" class="section-pad history-section">
      ...
      if (pinjamBukuId) {
          // Scroll tepat ke form borrowForm
          setTimeout(() => {
              const form = document.getElementById('borrowForm');
              if (form) {
    ...
    <section class="section-pad feature-highlight">
      <div class="mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <article class="mini-feature reveal" style="border-radius: 15px;">
                <i class="fa-solid fa-pen-to-square"></i><span>CRUD Koleksi Buku</span>
            </article>
            <article class="mini-feature reveal" style="border-radius: 15px;">
                <i class="fa-solid fa-user-pen"></i><span>Direktori Anggota</span>
            </article>
            <article class="mini-feature reveal" style="border-radius: 15px;">
                <i class="fa-solid fa-calendar-check"></i><span>Transaksi Peminjaman</span>
            </article>
            <article class="mini-feature reveal" style="border-radius: 15px;">
                <i class="fa-solid fa-rotate"></i><span>Trigger Stok Otomatis</span>
            </article>
            <article class="mini-feature reveal" style="border-radius: 15px;">
                <i class="fa-solid fa-database"></i><span>Arsip Layanan Terhubung</span>
            </article>
            <article class="mini-feature reveal" style="border-radius: 15px;">
                <i class="fa-solid fa-chart-pie"></i><span>Dashboard Petugas</span>
            </article>
        </div>
      </div>
    </section>
    <section id="cta" class="cta-section" style="
        width: calc(100% - 40px);
        margin: 20px auto;
        padding: 64px 20px;
        border-radius: 15px;
    ...
    ">
        <div style="max-width: 1200px; margin: 0 auto;">
            ...
            <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 16px; margin-top: 36px;">
  ...
  <footer id="kontak" class="footer-section px-5 py-12 lg:px-8" style="background: #071a35;">
