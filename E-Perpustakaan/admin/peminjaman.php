...
if ($action === 'delete') {
    ...
    error_log("=== DELETE PEMINJAMAN ===");
    error_log("ID yang diterima: " . $id);
    
    if (!$id) {
        error_log("ERROR: ID tidak valid!");
        $error = 'ID peminjaman tidak valid.';
    } else {
        try {
            // Cek apakah peminjaman ada
            $checkStmt = $pdo->prepare('SELECT * FROM peminjaman WHERE id_peminjaman = ?');
            $checkStmt->execute([$id]);
            $loan = $checkStmt->fetch();
            
            if (!$loan) {
                error_log("ERROR: Peminjaman dengan ID $id tidak ditemukan!");
                $error = 'Data peminjaman tidak ditemukan.';
            } else {
                error_log("Data ditemukan: " . print_r($loan, true));
                
                // Hapus peminjaman
                $statement = $pdo->prepare('DELETE FROM peminjaman WHERE id_peminjaman = ?');
                $statement->execute([$id]);
                
                error_log("SUKSES: Peminjaman ID $id berhasil dihapus");
...
<!-- Peminjaman -->
<form method="POST" onsubmit="showConfirmModal('⚠️ Yakin ingin menghapus transaksi ini?', this, this.querySelector('button')); return false;">
...
<div class="admin-pagination">
    <?= generatePagination($page, $totalPages, 'peminjaman.php?q=' . urlencode($search)) ?>
</div>
    </section>
  </main>
  <script>
