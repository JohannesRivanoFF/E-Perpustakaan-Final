...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save') {
            ...
            if ($nama === '' || $alamat === '' || !preg_match('/^[0-9]{9,15}$/', $noHp)) {
                throw new RuntimeException('Lengkapi data anggota dengan benar.');
            }

            if ($id) {
                $statement = $pdo->prepare('UPDATE anggota SET nama = ?, alamat = ?, no_hp = ? WHERE id_anggota = ?');
                $statement->execute([$nama, $alamat, $noHp, $id]);
                $message = 'Data anggota berhasil diperbarui.';
            } else {
                $statement = $pdo->prepare('INSERT INTO anggota (nama, alamat, no_hp) VALUES (?, ?, ?)');
                $statement->execute([$nama, $alamat, $noHp]);
...
if ($search !== '') {
    $whereSql = 'WHERE nama LIKE ? OR alamat LIKE ? OR no_hp LIKE ?';
  ...
  <main class="admin-main">
    ...
    <section class="admin-panel">
      ...
      <form method="GET" class="admin-filter">
        <input type="search" name="q" placeholder="Cari nama, alamat, atau nomor HP" value="<?= htmlspecialchars($search) ?>">
        <button class="secondary-button" type="submit">Cari</button>
      </form>
      <div class="admin-table-wrap">
        <table class="admin-table">
          <thead><tr><th>Nama</th><th>No HP</th><th>Alamat</th><th>Terdaftar</th><th>Aksi</th></thead>
          <tbody>
